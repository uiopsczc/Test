...
namespace CsCat
{
  public class ByteUtil
  {
    ...
    public static bool ToBoolean(byte[] value, int start_index = 0, bool is_net_order = false)
    {
      if (is_net_order)
        Array.Reverse(value);
      return BitConverter.ToBoolean(value, start_index);
    }

    public static char ToChar(byte[] value, int start_index = 0, bool is_net_order = false)
    {
      if (is_net_order)
        Array.Reverse(value);
      return BitConverter.ToChar(value, start_index);
    }


    public static short ToShort(byte[] value, int start_index = 0, bool is_net_order = false)
    {
      if (is_net_order)
        Array.Reverse(value);
      return BitConverter.ToInt16(value, start_index);
    }


    public static int ToInt(byte[] value, int start_index = 0, bool is_net_order = false)
    {
      if (is_net_order)
        Array.Reverse(value);
      return BitConverter.ToInt32(value, start_index);
    }


    public static long ToLong(byte[] value, int start_index = 0, bool is_net_order = false)
    {
      if (is_net_order)
        Array.Reverse(value);
      return BitConverter.ToInt64(value, start_index);
    }


    public static float ToFloat(byte[] value, int start_index = 0, bool is_net_order = false)
    {
      if (is_net_order)
        Array.Reverse(value);
      return BitConverter.ToSingle(value, start_index);
    }


    public static double ToDouble(byte[] value, int start_index = 0, bool is_net_order = false)
    {
      if (is_net_order)
        Array.Reverse(value);
      return BitConverter.ToDouble(value, start_index);
    }

    /// <summary>
    ///   将数字转化为bytes
    /// </summary>
    public static byte[] ToBytes(long value, int len, bool is_net_order = false)
    {
      if (is_net_order)
      {
        var bb = new byte[len];
