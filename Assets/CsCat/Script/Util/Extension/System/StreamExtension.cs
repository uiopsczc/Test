...
namespace CsCat
{
  public static class StreamExtension
  {
    ...
    public static int ReadBytes(this Stream self, byte[] buffer)
    {
      ...
      var offset = 0;
      var total_count = 0;
      var read_unit = buffer.Length > 100 ? 100 : buffer.Length;
      while (true)
      {
        var bytes_read_count = self.Read(buffer, offset,
          offset + read_unit > buffer.Length ? buffer.Length - offset : read_unit);
        if (bytes_read_count == 0)
          break;
        offset += bytes_read_count;
        total_count += bytes_read_count;
      }
