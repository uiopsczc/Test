...
namespace CsCat
{
  public class DoerEvent : Doer
  {
    ...
    public DoerEventDefinition GetDoerEventDefinition()
    {
      return GetDoerEventFactory().GetDoerEventDefinition(this.GetId());
    }
    ...
    public virtual bool Execute(string desc, Doer owner, DoerAttrParser doerAttrParser)
    {
      if (!CheckDoerEventTriggerCondition(doerAttrParser))
        return false;
      DoerEventDefinition definition = GetDoerEventDefinition();
      bool is_not_talk = definition.is_not_talk; // 不弹出talk
      //通用情况
      string trigger_desc = definition.trigger_desc;
      ...
      string[] step_ids = definition.step_ids;
      if (!step_ids.IsNullOrEmpty())
      {
        for (int i = 0; i < step_ids.Length; i++)
        {
          string step_id = step_ids[i];
          DoerEventStepDefinition stepDefinition = DefinitionManager.instance.doerEventStepDefinition.GetData(step_id);
          ok = ExecuteStep(desc + "步骤" + (i + 1), step_id, owner, doerAttrParser, word_list);
          if (ok == 0)
            break;
          else if (ok == 2 && stepDefinition.is_stop_here)
            break;
        }
      }
    ...
    {
      DoerEventDefinition definition = DefinitionManager.instance.doerEventDefinition.GetData(this.GetId());
      bool is_not_talk = definition.is_not_talk; // 不弹出talk
      string trigger_condition = definition.trigger_condition; // 触发条件
      if (!trigger_condition.IsNullOrWhiteSpace() && !doerAttrParser.ParseBoolean(trigger_condition)) //不满足触发的情况
      {
        if (!is_not_talk)
        {
          string can_not_trigger_desc = definition.can_not_trigger_desc;
    ...
    {
      DoerEventStepDefinition definition = DefinitionManager.instance.doerEventStepDefinition.GetData(doerEventStep_id);
      string trigger_condition = definition.trigger_condition; // 触发条件
      if (!trigger_condition.IsNullOrWhiteSpace() && !doerAttrParser.ParseBoolean(trigger_condition)) //不满足触发的情况
      {
        string can_not_trigger_desc = definition.can_not_trigger_desc;
      ...
      string trigger_desc = definition.trigger_desc; // 触发提示语
      if (!trigger_desc.IsNullOrWhiteSpace())
        word_list.Add(doerAttrParser.ParseString(trigger_desc));
      string execute_condition = definition.execute_condition; // 执行条件
      if (!execute_condition.IsNullOrWhiteSpace() && !doerAttrParser.ParseBoolean(execute_condition)) //不满足执行条件的情况
      {
        string can_not_execute_desc = definition.can_not_execute_desc; // 不执行提示语
        if (!can_not_execute_desc.IsNullOrWhiteSpace())
          word_list.Add(doerAttrParser.ParseString(can_not_execute_desc));
        return 1;
      }

      string execute_desc = definition.execute_desc; // 执行提示语
      ...
      //设置属性、更改属性
      Dictionary<string, string> set_attr_dict = definition.set_attr_dict;
      foreach (var attr_name in set_attr_dict.Keys)
        doerAttrSetter.Set(attr_name, set_attr_dict[attr_name], false);
      Dictionary<string, string> add_attr_dict = definition.add_attr_dict;
