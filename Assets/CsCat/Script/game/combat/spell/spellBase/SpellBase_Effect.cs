	...
	public partial class SpellBase
	{
		...
		public List<string> CreateHandEffect(float duration)
		{
			if (this.cfgSpellData._hand_effect_ids.IsNullOrEmpty())
				return null;
			var effect_ids = this.cfgSpellData._hand_effect_ids;
		...
		public List<string> CreateGoEffect(float duration)
		{
			if (this.cfgSpellData._go_effect_ids.IsNullOrEmpty())
				return null;
			var effect_ids = this.cfgSpellData._go_effect_ids;
			var guid_list = new List<string>();
			foreach (var effect_id in effect_ids)
			{
		...
		{
			if (this.cfgSpellData._hit_effect_ids.IsNullOrEmpty() && force_effect_id_list.IsNullOrEmpty())
				return null;
		...
		{
			if (this.cfgSpellData._ground_effect_ids.IsNullOrEmpty() && force_effect_id_list.IsNullOrEmpty())
				return null;
		...
		public List<string> CreateLineEffect(IPosition target_iposition, float speed, float acc_speed)
		{
			...
			if (target_iposition == null || !target_iposition.IsValid())
				return null;
			var effect_ids = this.cfgSpellData._line_effect_ids;
			var guid_list = new List<string>();
			foreach (var effect_id in effect_ids)
			{
