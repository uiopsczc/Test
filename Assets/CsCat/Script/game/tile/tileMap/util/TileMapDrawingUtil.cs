...
namespace CsCat
{
  public static class TileMapDrawingUtil
  {
    ...
    {
      int gridX = TileSetBrushUtil.GetGridX(vLocalPos, tileMap.cell_size);
      int gridY = TileSetBrushUtil.GetGridY(vLocalPos, tileMap.cell_size);
        ...
        )
        {
          ...
          while (check.Count > 0)
          {
                        ...
                        new Point(-1, 0), new Point(1, 0)})
            {
              ...
              {
                if (filledPoints[(next.Y - tilemap.min_grid_y) * tilemap.GridWidth + next.X - tilemap.min_grid_x]) continue; // skip already filled points
                uint nextTileData = tilemap.GetTileData(next.X, next.Y);
                if (
                    isBrush ?
                    TileSetUtil.GetTileSetBrushIdFromTileData(floodFrom) == TileSetUtil.GetTileSetBrushIdFromTileData(nextTileData)
    ...
    public static void DrawLine(TileMap tilemap, Vector2 locPosA, Vector2 locPosB, uint[,] tileData, bool randomize = false)
    {
    ...
    public static void DrawLine(TileMap tilemap, int x0, int y0, int x1, int y1, uint[,] tileData, bool randomize = false)
    {
    ...
    public static void DrawRect(TileMap tilemap, Vector2 locPosA, Vector2 locPosB, uint[,] tileData, bool isFilled, bool is9Sliced = false, bool randomize = false)
    {
      int x0 = TileMapUtil.GetGridX(tilemap, locPosA);
      int y0 = TileMapUtil.GetGridY(tilemap, locPosA);
      int x1 = TileMapUtil.GetGridX(tilemap, locPosB);
