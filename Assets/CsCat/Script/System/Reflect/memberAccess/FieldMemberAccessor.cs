...
namespace CsCat
{
  ...
  public sealed class FieldMemberAccessor : MemberAccessor
  {
    ...
    {
      var dynamicMethod = new DynamicMethod(fieldInfo.ReflectedType.FullName + ".get_" + fieldInfo.Name, typeof(object),
        new[]
        {
          typeof(object)
      ...
      if (fieldInfo.DeclaringType.IsValueType) //DeclaringType声明该fieldInfo所在的类（声明的地方，可能是父类声明，子类调用，但声明的地方是父类，所以指向的是父类）
    ...
    private void InitializeSetter(FieldInfo fieldInfo)
    {
      ...
      var dynamicMethod = new DynamicMethod(fieldInfo.ReflectedType.FullName + ".set_" + fieldInfo.Name, typeof(void),
        new[]
        {
          typeof(object),
          typeof(object)
