    ...
    public sealed class PropertyMemberAccessor : MemberAccessor
    {
        ...
        private void InitializeGetter(PropertyInfo propertyInfo)
        {
            ...
            var dynamicMethod = new DynamicMethod(
                propertyInfo.ReflectedType.FullName + ".get_property_" + propertyInfo.Name,
                typeof(object), new[]
            ...
            ilGenerator.EmitCall(OpCodes.Callvirt, propertyInfo.GetGetMethod(),
                null); //Callvirt  call virtual method  调用虚方法
        ...
        private void InitializeSetter(PropertyInfo propertyInfo)
        {
            //ReflectedType  如果filedInfo的类是内部定义类，则ReflectedType返回的是定义该内部内所在的类（即包含该内部类的类）
            var dynamicMethod = new DynamicMethod(
                propertyInfo.ReflectedType.FullName + ".set_property_" + propertyInfo.Name,
                typeof(void), new[]
                {
                    typeof(object),
                    typeof(object)
