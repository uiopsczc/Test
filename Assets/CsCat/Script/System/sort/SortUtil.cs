...
namespace CsCat
{
	public class SortUtil
	{
		...
		private static void __Merge<T>(IList<T> list, int left_index, int middle_index, int right_index,
			Func<T, T, bool> func)
		{
			...
			//处理没有被完全处理的数组
			for (int i= left_list_cur_index; i <= middle_index; i++)
				new_list.Add(list[i]);
			for (int i= right_list_cur_index; i <= right_index; i++)
				new_list.Add(list[i]);
		...
		private static void __Merge(IList list, int left_index, int middle_index, int right_index,
		...
		//则是将count由小到大排序，注意比较大小时不要漏掉等于号，否则相等时也进行排序，则排序不稳定
		public static void QuickSort(IList list, Func<object, object, bool> func, int? left_index = null,
		...
		///////////////////////////////////////////////////////////////////////////
		public static void QuickSort<T>(IList<T> list, Func<T, T, bool> func, int? left_index = null,
			int? right_index = null)
		{
			int _left_index = left_index.GetValueOrDefault(0);
			int _right_index = right_index.GetValueOrDefault(list.Count - 1);
		...
		private static int Partition<T>(IList<T> list, Func<T, T, bool> func, int left_index, int right_index)
		{
			...
			int partition_index = left_index;
			T temp;
			for (int i = partition_index; i < right_index; i++)
			{
				if (func(list[i], pivot_value))
				{
					//swap list[i] and list[partition_index]
			...
			//swap list[partition_index] and list[right]
			temp = list[partition_index];
			list[partition_index] = list[right_index];
