...
namespace CsCat
{
  ...
  {
    public ValueListDictionary<EventName, KeyValuePairCat<Action<P0, P1, P2, P3>, bool>> listener_dict =
      new ValueListDictionary<EventName, KeyValuePairCat<Action<P0, P1, P2, P3>, bool>>();


    public EventListenerInfo<P0, P1, P2, P3> AddListener(string eventName, Action<P0, P1, P2, P3> handler)
    {
      return AddListener(eventName.ToEventName(), handler);
    }

    public EventListenerInfo<P0, P1, P2, P3> AddListener(EventName eventName, Action<P0, P1, P2, P3> handler)
    {
      var handler_info = PoolCatManagerUtil.Spawn<KeyValuePairCat<Action<P0, P1, P2, P3>, bool>>().Init(handler, true);
      listener_dict.Add(eventName, handler_info);
      return PoolCatManagerUtil.Spawn<EventListenerInfo<P0, P1, P2, P3>>().Init(eventName, handler);
    }
    ...
    public bool RemoveListener(EventListenerInfo<P0, P1, P2, P3> eventListenerInfo)
    {
      return RemoveListener(eventListenerInfo.eventName, eventListenerInfo.handler);
    }

    public bool RemoveListener(EventName eventName, Action<P0, P1, P2, P3> handler)
    {
      try
      {
    ...
    public bool RemoveListener(Action<P0, P1, P2, P3> handler)
    {
      ...
      {
        var result = RemoveListener(eventName.Clone(), handler);
        if (result)
          return result;
      }
    ...
    {
      Broadcast(eventName.ToEventName(), p0, p1, p2, p3);
    }

    public void Broadcast(EventName eventName, P0 p0, P1 p1, P2 p2, P3 p3)
    {
      if (listener_dict.ContainsKey(eventName))
      {
        var handler_info_list = PoolCatManagerUtil.Spawn<List<KeyValuePairCat<Action<P0, P1, P2, P3>, bool>>>();
        handler_info_list.AddRange(listener_dict[eventName]);
        foreach (var handler_info in handler_info_list)
          try
          {
            if (handler_info.value)
              handler_info.key(p0, p1, p2, p3);
          }
          catch (Exception ex)
          {
            LogCat.LogError(ex);
