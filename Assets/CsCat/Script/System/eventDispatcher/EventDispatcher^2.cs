...
namespace CsCat
{
  ...
  {
    public ValueListDictionary<EventName, KeyValuePairCat<Action<P0, P1>, bool>> listener_dict =
      new ValueListDictionary<EventName, KeyValuePairCat<Action<P0, P1>, bool>>();


    public EventListenerInfo<P0, P1> AddListener(string eventName, Action<P0, P1> handler)
    {
    ...
    public bool RemoveListener(EventListenerInfo<P0, P1> eventListenerInfo)
    {
      return RemoveListener(eventListenerInfo.eventName, eventListenerInfo.handler);
    }

    public bool RemoveListener(EventName eventName, Action<P0, P1> handler)
    {
      try
      {
    ...
    {
      Broadcast(eventName.ToEventName(), p0, p1);
    }

    public void Broadcast(EventName eventName, P0 p0, P1 p1)
    {
      if (listener_dict.ContainsKey(eventName))
      {
        var handler_info_list = PoolCatManagerUtil.Spawn<List<KeyValuePairCat<Action<P0, P1>, bool>>>();
        handler_info_list.AddRange(listener_dict[eventName]);
        foreach (var handler_info in handler_info_list)
          try
          {
            if (handler_info.value)
              handler_info.key(p0, p1);
          }
          catch (Exception ex)
          {
            LogCat.LogError(ex);
