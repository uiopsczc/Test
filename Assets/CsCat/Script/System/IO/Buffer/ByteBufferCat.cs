...
namespace CsCat
{
  public class ByteBufferCat : MemoryStream
  {
    ...
    private readonly bool is_infinite;
    ...
    /// <returns></returns>
    private void CheckBounds(int offset, int len, int size)
    {
      if ((offset | len | (offset + len) | (size - (offset + len))) < 0)
        throw new IndexOutOfRangeException();

      if (len > Remaining() && is_infinite == false)
        throw new IndexOutOfRangeException();
    }
    ...
    public ByteBufferCat Put(ByteBufferCat src)
    {
      ...
      var n = src.Remaining();
      if (n > Remaining() && is_infinite == false)
        throw new IndexOutOfRangeException();
      for (var i = 0; i < n; i++)
        Put(src.Get());
      return this;
