...
namespace CsCat
{
  public partial class LogCat : MonoBehaviour
  {
    ...
    public static void Init()
    {
      ...
      if (Directory.Exists(LogCatConst.Log_Base_Path))
      {
        //每次启动客户端删除10天前保存的Log
        DateTime time = DateTimeUtil.NowDateTime();
        DateTime pass_time = time.AddDays(-10);
        foreach (string full_file_name in Directory.GetFiles(LogCatConst.Log_Base_Path))
        {
    ...
    private static void WriteLogFile(string content)
    {
      ...
      string file_path = LogCatConst.Log_Base_Path + day + ".txt";
