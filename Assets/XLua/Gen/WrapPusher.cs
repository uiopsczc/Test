...
namespace XLua
{
    public partial class ObjectTranslator
    {
        ...
        class IniterAdderUnityEngineVector2
        {
			...
			static void Init(LuaEnv luaenv, ObjectTranslator translator)
			{
				...
				translator.RegisterPushAndGetAndUpdate<CsCat.EUILayerName>(translator.PushCsCatEUILayerName, translator.Get, translator.UpdateCsCatEUILayerName);
        ...
        int CsCatEUILayerName_TypeID = -1;
		int CsCatEUILayerName_EnumRef = -1;
        
        public void PushCsCatEUILayerName(RealStatePtr L, CsCat.EUILayerName val)
        {
            if (CsCatEUILayerName_TypeID == -1)
            {
			    bool is_first;
                CsCatEUILayerName_TypeID = getTypeId(L, typeof(CsCat.EUILayerName), out is_first);
				
				if (CsCatEUILayerName_EnumRef == -1)
				{
				    Utils.LoadCSTable(L, typeof(CsCat.EUILayerName));
				    CsCatEUILayerName_EnumRef = LuaAPI.luaL_ref(L, LuaIndexes.LUA_REGISTRYINDEX);
				}
				
            }
			
			if (LuaAPI.xlua_tryget_cachedud(L, (int)val, CsCatEUILayerName_EnumRef) == 1)
            {
			    return;
			}
			
            IntPtr buff = LuaAPI.xlua_pushstruct(L, 4, CsCatEUILayerName_TypeID);
            if (!CopyByValue.Pack(buff, 0, (int)val))
            {
                throw new Exception("pack fail fail for CsCat.EUILayerName ,value="+val);
            }
			
			LuaAPI.lua_getref(L, CsCatEUILayerName_EnumRef);
			LuaAPI.lua_pushvalue(L, -2);
			LuaAPI.xlua_rawseti(L, -2, (int)val);
			LuaAPI.lua_pop(L, 1);
        ...
        public void Get(RealStatePtr L, int index, out CsCat.EUILayerName val)
        {
		    LuaTypes type = LuaAPI.lua_type(L, index);
            if (type == LuaTypes.LUA_TUSERDATA )
            {
			    if (LuaAPI.xlua_gettypeid(L, index) != CsCatEUILayerName_TypeID)
				{
				    throw new Exception("invalid userdata for CsCat.EUILayerName");
				}
                ...
                if (!CopyByValue.UnPack(buff, 0, out e))
                {
                    throw new Exception("unpack fail for CsCat.EUILayerName");
                }
				val = (CsCat.EUILayerName)e;
                
            }
            else
            {
                val = (CsCat.EUILayerName)objectCasters.GetCaster(typeof(CsCat.EUILayerName))(L, index, null);
            }
        }
		
        public void UpdateCsCatEUILayerName(RealStatePtr L, int index, CsCat.EUILayerName val)
        {
            ...
            if (LuaAPI.lua_type(L, index) == LuaTypes.LUA_TUSERDATA)
            {
                ...
                if (!CopyByValue.Pack(buff, 0,  (int)val))
                {
                    throw new Exception("pack fail for CsCat.EUILayerName ,value="+val);
                }
            }
	...
	public partial class StaticLuaCallbacks
    {
	    internal static bool __tryArrayGet(Type type, RealStatePtr L, ObjectTranslator translator, object obj, int index)
		{
			...
			else if (type == typeof(CsCat.EUILayerName[]))
			{
			    CsCat.EUILayerName[] array = obj as CsCat.EUILayerName[];
				translator.PushCsCatEUILayerName(L, array[index]);
				return true;
			}
