...
namespace CSObjectWrapEditor
{
  ...
  public static class Generator
  {
    ...
    {
      "op_Addition", "op_Subtraction", "op_Multiply", "op_Division", "op_Equality", "op_UnaryNegation", "op_LessThan",
      "op_LessThanOrEqual", "op_Modulus",
    ...
    static void getClassInfo(Type type, LuaTable parameters)
    {
      ...
      if (!type.IsAbstract)
      {
        foreach (var con in type.GetConstructors(BindingFlags.Instance | BindingFlags.Public | BindingFlags.IgnoreCase)
          .Cast<MethodBase>()
      ...
      parameters.Set("methods", type
        .GetMethods(BindingFlags.Public | BindingFlags.Instance | BindingFlags.Static | BindingFlags.IgnoreCase |
                    BindingFlags.DeclaredOnly)
        .Where(method => !isDefined(method, typeof(ExtensionAttribute)) ||
      ...
      parameters.Set("getters", type
        .GetProperties(BindingFlags.Public | BindingFlags.Instance | BindingFlags.Static | BindingFlags.IgnoreCase |
                       BindingFlags.DeclaredOnly)
      ...
      parameters.Set("events",
        type.GetEvents(BindingFlags.Public | BindingFlags.Instance | BindingFlags.Static | BindingFlags.IgnoreCase |
                       BindingFlags.DeclaredOnly).Where(e => !isObsolete(e) && !isMemberInBlackList(e))
    ...
    static void getInterfaceInfo(Type type, LuaTable parameters)
    {
      ...
      parameters.Set("methods", itfs.SelectMany(i => i.GetMethods())
        .Where(method => !method.IsSpecialName && !method.IsGenericMethod && !method.Name.StartsWith("op_") &&
                         ...
                         !method.Name
                           .StartsWith("remove_")) //GenericMethod can not be invoke becuase not static info available!
        .ToList());
      ...
      GenOne(null, (type, type_info) =>
      {
        ...
        foreach (var _type in types)
          type2fields.Set(_type,
            _type.GetFields(BindingFlags.Public | BindingFlags.Static).Where(x => !isMemberInBlackList(x)).ToArray());
    ...
    static void GenInterfaceBridge(IEnumerable<Type> types, string save_path)
    {
      foreach (var wrap_type in types)
      {
        ...
        string filePath = save_path + wrap_type.ToString().Replace("+", "").Replace(".", "")
                          ...
                          "Bridge.cs";
        StreamWriter textWriter = new StreamWriter(filePath, false, Encoding.UTF8);
        GenOne(wrap_type, (type, type_info) => { getInterfaceInfo(type, type_info); }, templateRef.LuaInterfaceBridge,
          textWriter);
        textWriter.Close();
      }
    }
    ...
    static void GenDelegateBridge(IEnumerable<Type> types, string save_path, IEnumerable<Type> hotfix_check_types)
    {
      ...
      foreach (var kv in HotfixCfg)
      {
        ...
        hotfxDelegates.AddRange(kv.Key.GetMethods(bindingAttrOfMethod)
          ...
          .Where(method => !ignoreProperty || !method.IsSpecialName ||
    ...
    static void AddToList(List<Type> list, Func<object> get, object attr)
    {
      ...
      {
        throw new InvalidOperationException("Only field/property with the type IEnumerable<Type> can be marked " +
                                            attr.GetType().Name);
      }
#if XLUA_GENERAL
...
#pragma warning restore 618
        ...
        if (cfg is IEnumerable<Type>)
        {
          ...
          foreach (var type in cfg as IEnumerable<Type>)
          {
            if (!HotfixCfg.ContainsKey(type) && !isObsolete(type)
                                             ...
                                             && (type.Namespace == null ||
                                                 (type.Namespace != "XLua" && !type.Namespace.StartsWith("XLua.")))
    ...
    public static void Gen(IEnumerable<Type> wraps, IEnumerable<Type> gc_optimze_list, IEnumerable<Type> itf_bridges,
      string save_path)
    {
      templateCache.Clear();
