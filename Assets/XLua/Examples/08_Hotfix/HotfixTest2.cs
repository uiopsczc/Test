...
namespace XLuaTest
{
  ...
  public class HotfixTest2 : MonoBehaviour
  {
    ...
    void Start()
    {
      ...
      var start = DateTimeUtil.NowDateTime();
      for (int i = 0; i < CALL_TIME; i++)
      {
        calc.Add(2, 1);
      }
      var d1 = (DateTimeUtil.NowDateTime() - start).TotalMilliseconds;
      Debug.Log("Hotfix using:" + d1);

      start = DateTimeUtil.NowDateTime();
      for (int i = 0; i < CALL_TIME; i++)
      {
        ordinaryCalc.Add(2, 1);
      }
      var d2 = (DateTimeUtil.NowDateTime() - start).TotalMilliseconds;
