...
public class LocalToLuaIDE : Editor
{
    ...
    [MenuItem("EmmyLua/Set Your IDE Path")]
    ...
    [MenuItem("EmmyLua/Set Your Luaproject Root")]
    ...
    public static string HanlePath(string path)
    {
        path = path.Substring(0, path.Length - BuildConst.Lua_Suffix.Length);
        path= path.Replace(".", "/");
        return path + BuildConst.Lua_Suffix;
    }
    ...
    private static string GetListViewRowCount(ref int line)
    {
        ...
        int index = condition.IndexOf(string.Format("{0}:",BuildConst.Lua_Suffix));
        if (index >= 0)
        {
            int start = condition.IndexOf("[");
            int end = condition.IndexOf("]:");
            string _line = condition.Substring(index + (BuildConst.Lua_Suffix.Length+1), end - index - (BuildConst.Lua_Suffix.Length + 1)); ///这里取的行数只适配xlua
            Int32.TryParse(_line, out line);
            return HanlePath(condition.Substring(start + 1, index + (BuildConst.Lua_Suffix.Length - 1) - start));
        }

        index = condition.IndexEndOf("LuaException: ");
        if (index >= 0)
        {
            return HanlePath(ParseLuaRowCount_LuaException(condition.Substring(index+1), out line));
        }
        ...
        if (index >= 0)
        {
            int start = condition.IndexOf("[");
            int end = condition.IndexOf("]:");
            string _line = condition.Substring(index + 5, end - index - 5);///这里取的行数只适配xlua
            Int32.TryParse(_line, out line);
    ...
    private static string ParseLuaRowCount_LuaException(string text, out int line)
    {
        int startIndex = 0;
        int endIndex = text.IndexOf(":");
        string fileNameStr = text.Substring(startIndex , endIndex - startIndex) + BuildConst.Lua_Suffix;
        int lineTxtStartIndex = endIndex;
        int lineTxtEndIndex = text.IndexOf(":", endIndex + 1);
        string _line = text.Substring(lineTxtStartIndex + 1, lineTxtEndIndex- lineTxtStartIndex-1); ///这里取的行数只适配xlua
        Int32.TryParse(_line, out line);
        return fileNameStr;
    }
    ...
    private static string GetLog()
    {
        ...
        LogEntriesGetEntry.Invoke(new object(), new object[] { row, logEntry });
